...
function ListSection({ booksData, onSetBooksList, currentBookData, onSetCurrentData }) {
    ...
    <ul className="content-list">
      {booksData.length > 0 ? (
        booksData.map((book, index) => (
          <li key={book.isbn} onClick={() => onSetCurrentData({ index, ...book })}>
            <div>
              <p className="id">
              ...
              <p className="field5">{book.publicationDate.toDateString()}</p>
              <button
